...
Zamana gecikmeli enumeration için [bu kod](https://github.com/HasanFiratKilic/Web-Hacking/blob/main/Authentication/cods/BForce_and_Enumeration.py)  kullanılır. Bu kod dayasıyla aynı konumda kayıtlı username.txt dosyasındaki kullanıcıları tek tek dener ve 2 saniye geç gelen yanıtı potansiyel kullanıcı olarak ekranda gösterir. Lab bir kaç yanlış girişde ip engellemesi gerçekleştirir kod her denemede farklı bir ip ile sorgu gerçekleştirerek ip engellemesini atlatır. Kodu çalıştırdığımızda `ai` adlı kullanıcıyı bulmuş olduk.
